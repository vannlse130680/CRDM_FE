  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Client manage</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        if (this.editedIndex > -1) {
          ...
          axios.post('http://172.16.189.126:8080/client', this.editedItem)
            .then(response => {
              ...
              if (response.status === 200) { this.desserts.push(this.customizeObj(response)) }
            })
            .catch(e => {
              this.errors.push(e)
            })
      ...
      view () {
        this.$router.push({ path: '/project/client-view' })
      },
    },
  }
</script>
